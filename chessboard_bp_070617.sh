...
#get data
#load('/work/blehne/METHQTL/SIGSUG/OLD3_070214/sigproces_110214.RData')  #contains marker densities
#load('/work/blehne/METHQTL/ANNO/anno_170714.RData')
load('/project/lolipop_b/METHQTL2/SIGSUG/OLD3_070214/sigproces_110214.RData')  #contains marker densities
load('/project/lolipop_b/METHQTL2/ANNO/anno_170714.RData')
#load('/project/lolipop_b/METHQTL2/REPL2/cosmopairs_combined_151216.RData')
#m1=merge(cosmo, annometh, by.x='cpg', by.y='rsid')
#m2=merge(m1, annosnp, by.x='snp', by.y='rsid')
#sigtab.cis=m2[m2$snp.chr==m2$cpg.chr & abs(m2$snp.pos-m2$cpg.pos)<=1000000,c('gpos.x', 'gpos.y')]
#sigtab.longrange=m2[m2$snp.chr==m2$cpg.chr & abs(m2$snp.pos-m2$cpg.pos)>1000000,c('gpos.x', 'gpos.y')]
#sigtab.trans=m2[m2$snp.chr!=m2$cpg.chr ,c('gpos.x', 'gpos.y')]
#colnames(sigtab.cis)=c('gpos.cpg', 'gpos.snp')
#colnames(sigtab.longrange)=c('gpos.cpg', 'gpos.snp')
#colnames(sigtab.trans)=c('gpos.cpg', 'gpos.snp')
#save(sigtab.cis, sigtab.trans, sigtab.longrange, file='/project/lolipop_b/METHQTL2/REPL2/cosmopairs_chessboard_070617.RData')
load('/project/lolipop_b/METHQTL2/REPL2/cosmopairs_chessboard_070617.RData')
chrborders=cumuchrl[1:23]
tckpos=rep(NA,22)
tckpos[1]=chrborders[2]/2
for(i in 2:22){tckpos[i] =chrborders[i] + (chrl[i]/2)}
color= rep(c('#0000FF20', 'white'), (length(chrborders)+1))
mn=win$mn
sn=win$sn
win.dens=cbind(mn,sn)
res.dens=data.frame(cbind(win$c,ceiling(((win$egw-win$sgw)/2)+win$sgw), win.dens))
colnames(res.dens)[1:2]=c('c','g')



#plot--------------------------------------------------------------------------------------------------------------
#png('/home/blehne/METHQTL/PLOTS/chessboard_070617.png', width = 3000, height=3000, units ='px')
#png('/home/mcloh/chessboard.png', width = 3000, height=3000, units ='px')
pdf(file="/home/mcloh/chessboard.pdf", height=18, width=18)
par(mar=c(3,3,3,3), mgp = c(2.5,1,0), oma=c(30,30,10,10), cex.axis=5, col.axis='dark grey', cex.lab=6)
...
#SNP density
plot(c(0,max(res.dens$sn)),c(chrborders[1],chrborders[23]), col='white', xlab='', ylab='', yaxt='n', xaxt='n', yaxs = 'i')
for(chr in 1:22){
